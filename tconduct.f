...
C Magnetic Field and Temperature at (i+1/2,j)
      dimension :: b2mx(1:i0,1:j0),temx(1:i0,1:j0)
C Magnetic Field and Temperature at (i,j+1/2)
      dimension :: b2my(1:i0,1:j0),temy(1:i0,1:j0)
...
C Calculate Magnetic Field and Temperature at (i+1/2,j)
      ...
      do i=1,i0-1
         ...
         b2mx(i,j) = bxm(i,j)**2 + bymx(i,j)**2 + bzmx(i,j)**2
...
C Calculate Magnetic Field and Temperature at (i,j+1/2)
      ...
      do i=1,i0
         ...
         b2my(i,j) = bxmy(i,j)**2 + bym(i,j)**2 + bzmy(i,j)**2
...
C Usually is=2,ie=i0-1,js=2,je=j0-1
      do j=js,je
      do i=is,ie
         rkappam1 = bxm(i,j)*bymx(i,j)*temx(i,j)**2.5/b2mx(i,j)
         rkappam2 = bxm(i-1,j)*bymx(i-1,j)*temx(i-1,j)**2.5/b2mx(i-1,j)
         rkappam3 = bxmy(i,j)*bym(i,j)*temy(i,j)**2.5/b2my(i,j)
         rkappam4 = bxmy(i,j-1)*bym(i,j-1)*temy(i,j-1)**2.5/b2my(i,j-1)

         rkappama = dym(j-1)*dxi(i)*bxm(i,j)**2*temx(i,j)**2.5
     &/b2mx(i,j)
         rkappamb = dym(j-1)*dxi(i-1)*bxm(i-1,j)**2*temx(i-1,j)**2.5
     &/b2mx(i-1,j)
         rkappamc = dxm(i-1)*dyi(j)*bym(i,j)**2*temy(i,j)**2.5
     &/b2my(i,j)
         rkappamd = dxm(i-1)*dyi(j-1)*bym(i,j-1)**2*temy(i,j-1)**2.5
     &/b2my(i,j-1)
